...
/// Runs the describe operation: reads the one `describe_request`, negotiates a
/// shared protocol version, and answers with this Field's manifest.
///
/// A Field that supports no version core offered emits no manifest at all -- a
/// manifest it cannot express correctly is worse than none -- and exits with
/// the negotiation code (A2 section 2). A describe run makes no Graph call,
/// holds no token, and needs no credential grant.
pub(crate) fn run(mut input: impl BufRead) -> ExitCode {
    ...
    .is_none()
    {
        crate::report(&format!(
            "{}: protocol version mismatch: core offered {:?}, this build supports \
             [{PROTOCOL_VERSION}]. Upgrade Fieldnotes or install a matching Field build.",
            crate::FIELD_NAME,
            request.supported_protocol_versions.as_slice()
        ));
    ...
    if emitter.manifest(manifest) {
        ...
        crate::report(&format!("{}: {detail}", crate::FIELD_NAME));
