...
pub(crate) fn run(mut input: impl BufRead) -> ExitCode {
    ...
    {
        crate::report(&format!(
            "fieldnotes-field-local: protocol version mismatch: core offered {:?}, this build \
             supports [{PROTOCOL_VERSION}]. Upgrade Fieldnotes or install a matching Field build.",
            request.supported_protocol_versions.as_slice()
        ));
    ...
    if emitter.manifest(manifest) {
        ...
        crate::report(&format!("fieldnotes-field-local: {detail}"));
