...
A notebook collected from live Microsoft 365 data (328 records: 248 contacts, 50 mail, 30 calendar events) showed that A1's single Note record cannot simultaneously be the machine record of what a source said, the human-readable artifact a person opens, and the evidence every derived record cites. [A3](approvals/A3-signals-and-notes.md) proposes splitting it: a **signal** for the collected evidence, and a **note** rendered from it by a type-specific template, with human-authored notes remaining first-class. A note links to its signal through a `signals` list carrying exactly one identifier in v0.1, and a given signal identifier appears on at most one note, so coverage is a map lookup rather than a search. A3 requires a disposable signal-to-note index under `.fieldnotes/cache/` — ADR 0001's fourth state class — because a mailbox-scale notebook cannot afford a scan of `notes/` per sync.

If approved, A3 would restate the first invariant above (canonical becomes signals, authored notes, and retained artifacts, with derived notes joining the disposable class), redefine a Note in `product.md`, restate ADR 0001's first state class, invalidate R0's golden fixtures, and restate both R0 and R8 over signals rather than Notes. A3 carries the exact proposed wording for each of those five statements rather than relying on a standing equivalence between "Note" and "signal". It leaves A2 and all three Microsoft Fields unchanged, keeps `notes/` flat, and its migration is a re-sync rather than a migrator, because deleting a refetchable notebook is already a supported lifecycle action.

The owner has settled the decision that touches the invariant "Fieldnotes represents the current collected state, not an append-only history ledger": **current state only**. Change observations recorded at detection time are rejected outright, and versioned signals are a possible later package rather than part of A3. A note may surface disagreement between concurrent current sources; it cannot report that a value changed. A3 also does not solve the defect that a contact's emails and phones are not typed, role-bearing properties — that needs an A2 amendment admitting nested structure, which A3 explicitly does not make.
