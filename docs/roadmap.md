...
- Enhancement is in-note. Collect must succeed without a model (deterministic scaffold). A small local LLM is on by default once installed and may be turned off. Compile and CI stay free of model downloads and GPU. Bring-your-own inference is not shipped in v0.1 and is no longer a hard product prohibition.
...
A notebook collected from live Microsoft 365 data (328 records: 248 contacts, 50 mail, 30 calendar events) showed that A1's single Note record cannot simultaneously be the machine record of what a source said, the human-readable artifact a person opens, and the evidence every derived record cites. [A3](approvals/A3-signals-and-notes.md) proposes splitting it: a **signal** for collected evidence, stored privately under `.fieldnotes/signals/<field-id>/`, and a **note** as the public reading artifact. A note links to its signal through a `signals` list carrying exactly one identifier in v0.1. Collect skips an existing note unless `--force`; `--force` keeps the stored slug. Contacts are not notes — that Field writes identifier working data for the graph to match on. A3 requires a disposable signal-to-note index under `.fieldnotes/cache/` because a mailbox-scale working set cannot afford a scan of `notes/` per sync.

If approved, A3 would restate the first invariant above, redefine a Note in `product.md`, restate ADR 0001's first state class to distinguish the public reading surface from private portable evidence, invalidate R0's golden fixtures, and restate R0 and R8. A3 carries the exact proposed wording rather than a standing "Note means signal" equivalence. It does not change the A2 protocol. It does change the Contacts Field's product role. Migration is a re-sync rather than a migrator.

The owner has settled **current state only**. Change observations recorded at detection time are rejected outright. A3 also does not solve nested contact methods — that needs an A2 amendment, which A3 does not make — and it no longer pretends contact markdown files were the right product.
